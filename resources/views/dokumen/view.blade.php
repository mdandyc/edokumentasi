	...
	<section class="wrapper scrollable">
		...
		<div class="main-grid">
					  ...
					  <form action="/dokumen/search" method="get">
						...
						<tbody>
						  ...
						  <tr>
							...
							<td>{{$dokumen->kategori->jenis_kategori}}</td>
			...
			@if (Auth::user()->username == $dokumen->usernameFK)
										<li><a href="/dokumen/{{$dokumen->id}}/edit" class="hvr-icon-up col-7">Edit</a></li>
										
										<li><a href="#" class="hvr-icon-shrink col-5" onclick="event.preventDefault();
                                                     document.getElementById('delete-form').submit(); if(!confirm('Are you sure want to delete this?')){return false;}" >Delete</a><form action="/dokumen/{{$dokumen->id}}" method="POST" id="delete-form" style="display:none;">
		...
		<li><a href="/dokumen/{{$dokumen->id}}/edit" class="hvr-icon-up col-7">Edit</a></li>
			...
			@endif
										<li><a href="/file/{{$dokumen->nama_dokumen}}" class="hvr-icon-down col-10">Download</a></li>
								</ul>
							</td>
						  </tr>
